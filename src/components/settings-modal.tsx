...
export function SettingsModal({ isOpen, onClose }: SettingsModalProps) {
  ...
  const [localAutoRefresh, setLocalAutoRefresh] = useState(settings.autoRefresh);
  const [localRefreshInterval, setLocalRefreshInterval] = useState(settings.refreshInterval);
  ...
  useEffect(() => {
    ...
    setLocalAutoRefresh(settings.autoRefresh);
    setLocalRefreshInterval(settings.refreshInterval);
  ...
  const handleSave = () => {
    updateSettings({
      ...
      refreshInterval: Math.max(30, localRefreshInterval),
    });
    onClose();
  };
  ...
  const handleReset = async () => {
    ...
    if (confirmed) {
      ...
      setLocalAutoRefresh(DEFAULT_SETTINGS.autoRefresh);
      setLocalRefreshInterval(DEFAULT_SETTINGS.refreshInterval);
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <label className="text-sm font-semibold text-foreground/80">Auto-refresh Dashboard</label>
                        {localAutoRefresh && (
                      ...
                      <p className="text-xs text-muted leading-relaxed">
                        Enable automatic hard-refresh of the dashboard page.
                      </p>
                    </div>
                    ...
                    {localAutoRefresh && (
                      ...
                      >
                        <div className="pt-4 border-t border-border/50">
                          <label className="text-sm font-semibold text-foreground/80 block mb-1">Refresh Interval (Seconds)</label>
                          <p className="text-xs text-muted leading-relaxed mb-4">
                            Set the interval for automatic page refresh (Minimum 30 seconds).
                          </p>
                          <div className="flex items-center gap-4">
                            <input
                              type="number"
                              ...
                              className="w-32 bg-background border border-border rounded-lg px-4 py-2.5 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all"
                            />
