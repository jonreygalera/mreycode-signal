...
export const WIDGET_DOCS: DocSection[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "signal.condition", type: "'above' | 'below' | 'equals' | 'diff'", description: "Threshold logic.", required: true },
      { key: "signal.threshold", type: "number", description: "The value that triggers the alert.", required: true },
      { key: "signal.action", type: "Array<'notify' | 'pulse' | 'sound' | 'notify-in-app' | 'webhook'>", description: "What happens when triggered.", required: true },
      ...
      { key: "signal.cooldown", type: "number", description: "Minutes to wait before re-triggering (Default: 0)." },
      { key: "signal.webhook", type: "WebhookConfig", description: "Configuration for outbound HTTP requests." },
      { key: "signal.webhook.url", type: "string", description: "The destination URL for the webhook trigger." },
      { key: "signal.webhook.method", type: "'GET' | 'POST' | 'PUT' | 'PATCH' | 'DELETE'", description: "HTTP method to use for the call." },
      { key: "signal.webhook.headers", type: "object | string", description: "Custom headers for the request." },
      { key: "signal.webhook.body", type: "string", description: "The request payload (JSON stringified)." },
    ],
  },
];
